...
in {
  ...
  prepareLib = {
    ...
    pkgs ? null,
    libSubdirs ? [ "lib" "tools" "utils" ],
  }: let
    files = concatLists (exploreDir roots (it: rec {
      ...
      out = for (findFiles (hasPostfix "nix") it.path) (
        ...
        in {
          level =
            if isAttrs attrs-or-func
            then 0
            else if elem "more" (it.breadcrumbs ++ [it.name])
            then 2
            else 1;
          path = fpath;
        }
