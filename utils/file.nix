...
in rec {
  ...
  isNonEmptyDir = path:
    if builtins.pathExists path && builtins.typeOf path == "path"
    then let
  ...
  underDir = directoryPath: path: match ("^" + toString directoryPath + "/.*") (toString path) != null;
  ...
  lsDirPaths = (
    path: map (subdir: path + "/${subdir}") (lsDirs path)
  );
  ...
  elemAt = index:
    if index < 0
    then list: builtins.elemAt list (length list + index)
    else list: builtins.elemAt list index;
  ...
  exploreDir = roots: picker: let
    recur = root: current: picker: depth: breadcrumbs:
      concatMap (
        name: let
          path = current + ("/" + name);
          t = picker {
        ...
        in
          ...
          ++ (
            if t.into
            then (recur root path picker (depth + 1) (breadcrumbs ++ [name]))
            else []
          )
      ) (lsDirsAll current);
  in
  ...
  findSubDirsContains = root: filename: let
  ...
  scanFilesByPostfix = postfix: dir:
    if builtins.pathExists dir
    then (findFilesRec (hasPostfix postfix) dir)
    else [];
}
