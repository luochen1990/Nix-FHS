...
in {
  ...
  mkFlake = mkFlakeArgs @ {
    ...
    nixpkgs,
    inputs ? self.inputs,
    roots ? [self.outPath] ++ filter pathExists [(self.outPath + "/nix") (self.outPath + "/private")],
    ...
    outline.packages = rec {
      subdirs = ["pkgs" "packages"];
      judge = x: elem x subdirs;
    };

    outline.nixosModules = rec {
      subdirs = ["modules" "nixosModules"];
      judge = x: elem x subdirs;
    };

    outline.nixosConfigurations = rec {
      subdirs = ["profiles" "hosts" "nixosConfigurations"];
      judge = x: elem x subdirs;
    };

    outline.devShells = rec {
      subdirs = ["shells" "devShells"];
      judge = x: elem x subdirs;
    };
  ...
  in {
    ...
    checks = eachSystem (
      context: let
        ...
        fileChecks = concatFor roots (
          ...
          in
            if pathExists checksPath
            then
              for (lsFiles checksPath) (
                name: let
                  checkPath = checksPath + "/${name}";
                in
                  if builtins.match ".*\\.nix$" name != null && name != "default.nix"
                  then {
    ...
    templates = let
      readTemplatesFromRoot = root: let
        ...
        then
          for (lsDirs templatePath) (
            name: let
              ...
              description =
                if hasFlake
                then (import flakePath).description or "Template: ${name}"
                else "Template: ${name}";
            in {
              inherit name;
              value = {
