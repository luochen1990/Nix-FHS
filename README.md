Nix Flake Hierarchy Standard (FlakeFHS)
===

FlakeFHS 是一个面向 Nix flake 的目录规范，它同时提供一个默认的 `flake.nix` 实现（`mkFlake`）。
用户几乎不需要自己编写 `flake.nix`。只需将 Nix 代码放置在约定的目录结构中，FlakeFHS 就会自动映射并生成所有对应的 flake outputs。

它约定了 flake 项目的目录布局。

FlakeFHS 致力于解决以下核心问题：
